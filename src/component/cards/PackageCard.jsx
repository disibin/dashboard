      ...
      <div className="group flex flex-col justify-between bg-white rounded-xl overflow-hidden box-border h-full border border-slate-100 shadow-xs hover:shadow-md transition-all duration-300 relative">
        <div className="flex flex-col justify-between grow">
          <div className="p-3 flex flex-col gap-3 grow">
            ...
            {pkg?.description && (
              <div
                className="text-sm leading-normal text-gray-700 m-0 font-poppins line-clamp-3 prose prose-slate max-w-none"
                dangerouslySetInnerHTML={{ __html: pkg.description }}
              />
            )}

            {Array.isArray(pkg.features) && pkg.features.length > 0 && (
              <div className="space-y-1.5 border-t border-slate-100 pt-2.5 mt-auto">
        ...
        {isStaff ? (
          ...
          <div className="p-3 border-t border-slate-100 bg-white">
            <button
              ...
              className="w-full inline-flex items-center justify-center gap-2 px-4 py-2 rounded-xl bg-slate-900 text-white font-semibold text-xs hover:bg-primary transition-all shadow-xs cursor-pointer disabled:opacity-50"
            >
