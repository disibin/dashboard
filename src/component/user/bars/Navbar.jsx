  ...
  return (
    <header className="w-full h-14 fixed top-0 left-0 right-0 px-4 sm:px-6 bg-white/90 backdrop-blur-md border-b border-slate-100 shadow-sm flex items-center justify-between z-50">
      <div className="flex items-center gap-3">
        ...
        >
          <FiSidebar size={18} className={`transition-transform duration-200 ${userSidebar ? 'rotate-180 text-primary' : 'text-slate-500'}`} />
          <span className="hidden sm:inline text-xs font-semibold tracking-tight">
            Menu
          </span>
        </button>

        <Link href="/user" className="text-lg font-bold text-slate-900 tracking-tight hidden sm:block">
          Dashboard
        </Link>
      </div>
