...
const ROLES = [
  ...
  },
  {
    value: 'manager',
    label: 'Manager',
    description: 'Full access to manage staff, leads, products, and operations.',
...
const NewStaffMemberForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await axios.post('/api/staff/new', { name, email, phone: phone || undefined, role });
      if (res.data.success) {
        toast.success('Staff member created! Invitation email sent.');
        setForm({ name: '', email: '', phone: '', role: '' });
        setTimeout(() => router.push('/panel/staff-member'), 1200);
      } else {
