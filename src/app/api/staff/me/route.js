...
export async function PATCH(req) {
    try {
        ...
        const res = await dbQuery(
            `UPDATE staffs SET ${setClause}, updated_at = now() WHERE id = $${keys.length + 1} RETURNING id, name, email, phone, role, city, country, address_line1, address_line2, state, postal_code`,
            [...values, staffId]
        );
...
export async function DELETE(req) {
    try {
        ...
        const staffRes = await dbQuery("SELECT password, role, is_active FROM staffs WHERE id = $1", [staffId]);
