...
export async function POST(req) {
    try {
        ...
        // Authenticated team security actions
        ...
        if (action === 'toggle-2fa') {
            ...
            await dbQuery(
                "UPDATE teams SET is_2fa_active = $1, updated_at = now() WHERE id = $2",
                [!!is2faActive, teamId]
            );
        ...
        if (action === 'change-password') {
            ...
            const teamRes = await dbQuery("SELECT password FROM teams WHERE id = $1", [teamId]);
            if (teamRes.rows.length === 0) {
                return NextResponse.json({ success: false, message: "Team member not found" }, { status: 404 });
            }

            const isMatch = await bcrypt.compare(currentPassword, teamRes.rows[0].password);
            if (!isMatch) {
                return NextResponse.json({ success: false, message: "Current password is incorrect" }, { status: 400 });
            }

            const hashedPassword = await bcrypt.hash(newPassword, 10);
            await dbQuery("UPDATE teams SET password = $1, updated_at = now() WHERE id = $2", [hashedPassword, teamId]);
        ...
        if (action === 'request-email-change') {
            ...
            const currentTeamRes = await dbQuery("SELECT email, name FROM teams WHERE id = $1", [teamId]);
            ...
            // Check uniqueness in both teams and users
            const existingTeam = await dbQuery("SELECT id FROM teams WHERE email = $1", [newEmail]);
            if (existingTeam.rows.length > 0) {
                return NextResponse.json({ success: false, message: "This email address is already in use by another team account" }, { status: 400 });
            }
            ...
            await dbQuery(
                `UPDATE teams 
                 ...
                 WHERE id = $4`,
                [newEmail, verificationCode, expiresAt, teamId]
            );
            ...
            const htmlContent = `
                <div style="font-family: sans-serif; max-width: 600px; margin: auto; padding: 40px; border: 1px solid #f0f0f0; border-radius: 12px; background: #ffffff;">
                    <h1 style="color: #0f172a; font-size: 22px; font-weight: 700; margin-bottom: 12px;">Team Email Change Verification Code</h1>
                    <p style="color: #475569; font-size: 14px; line-height: 1.6;">Hello ${teamName},</p>
                    <p style="color: #475569; font-size: 14px; line-height: 1.6;">A request was received to update your Disibin Team email address to <strong>${newEmail}</strong>.</p>
                    ...
                    <p style="color: #94a3b8; font-size: 12px; line-height: 1.5;">This code will expire in 15 minutes. If you did not request this email change, please notify your team manager immediately.</p>
                </div>
            `;

            await sendEmail({
                to: currentEmail,
                subject: "Disibin Team Email Change Verification Code",
                htmlContent
            });
        ...
        if (action === 'verify-email-change') {
            ...
            const { pending_email, email_change_code, email_change_expires_at } = teamRes.rows[0];
            ...
            await dbQuery(
                `UPDATE teams 
                 ...
                 WHERE id = $2`,
                [pending_email, teamId]
            );

            return NextResponse.json({
                success: true,
                message: "Team email address updated successfully!",
