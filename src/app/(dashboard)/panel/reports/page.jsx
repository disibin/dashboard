...
export default function TeamReportsPage() {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await axios.delete(`/api/staff/reports?id=${id}`);
      if (res.data.success) {
    ...
    <div className="p-4 w-full space-y-4">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-gradient-to-r from-amber-500/10 via-amber-500/5 to-transparent p-4 rounded-3xl border border-amber-500/20">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-semibold text-slate-900 tracking-tight">Issue Reports Inbox</h1>
        ...
        <button
          ...
          className="flex items-center gap-2 px-4 py-2.5 bg-white border border-slate-200 rounded-xl text-xs font-semibold text-slate-700 hover:bg-slate-50 transition-all shadow-sm self-start md:self-auto"
        >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm flex items-center justify-between">
          <div>
            ...
            <h3 className="text-2xl font-semibold text-slate-900 mt-1">{reports.length}</h3>
          </div>
          <div className="w-10 h-10 rounded-xl bg-slate-100 text-slate-600 flex items-center justify-center font-semibold">
            {reports.length}
          </div>
        </div>
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm flex items-center justify-between">
          <div>
            <span className="text-xs font-semibold uppercase tracking-wider text-secondary">Pending Review</span>
            <h3 className="text-2xl font-semibold text-secondary mt-1">{pendingCount}</h3>
          </div>
          <div className="w-10 h-10 rounded-xl bg-secondary/10 text-secondary flex items-center justify-center font-semibold">
            <FiClock size={18} />
          </div>
        </div>
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm flex items-center justify-between">
          <div>
            <span className="text-xs font-semibold uppercase tracking-wider text-primary">Replied / Resolved</span>
            <h3 className="text-2xl font-semibold text-primary mt-1">{repliedCount}</h3>
          </div>
