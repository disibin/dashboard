...
import RefundPolicyForm from '@/component/staff/forms/RefundPolicyForm';

export default function TeamRefundPolicyPage() {
  ...
  const fetchRefunds = async () => {
    setLoading(true);
    try {
      const res = await axios.get('/api/staff/refund-policy');
      if (res.data.success) {
        setData(res.data.data);
      }
    } catch (error) {
      if (error.response?.status === 403) {
        toast.error('Access Denied: Only Managers can manage Refund Policy');
      } else {
        toast.error('Failed to load Refund Policy entries');
      }
    } finally {
      setLoading(false);
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingItem) {
        const res = await axios.put('/api/staff/refund-policy', {
          id: editingItem.id,
          ...formData
        });
        if (res.data.success) {
          toast.success('Refund Policy item updated successfully');
          setShowForm(false);
          setEditingItem(null);
          fetchRefunds();
        }
      } else {
        const res = await axios.post('/api/staff/refund-policy', formData);
        if (res.data.success) {
          toast.success('Refund Policy item created successfully');
          setShowForm(false);
          fetchRefunds();
        }
      }
  ...
  const handleDelete = async (id) => {
    if (!window.confirm('Are you sure you want to delete this Refund Policy item?')) return;
    setDeletingId(id);
    try {
      const res = await axios.delete(`/api/staff/refund-policy?id=${id}`);
      if (res.data.success) {
  ...
  return (
    <div className="p-4 w-full min-h-screen pb-24">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8  p-4 sm:p-5 rounded-3xl text-primary shadow-sm">
        <div>
          <h1 className="text-2xl sm:text-4xl font-semibold tracking-tight">Refund Policy Management</h1>
          <p className=" text-sm mt-1 max-w-xl">
            Create, update, and publish official Refund Policy & SLA items. Restricted exclusively to authorized managers.
          </p>
        </div>

        <button
          onClick={handleOpenCreate}
          className="inline-flex items-center justify-center gap-2 px-5 py-3 rounded-2xl  text-sm font-semibold transition-all shadow-sm cursor-pointer shrink-0"
        >
          <FiPlus className="w-5 h-5 text-primary" /> Create Refund Policy Item
        </button>
      </div>
      ...
      <div className="flex items-center justify-between gap-4 mb-6 border-b border-slate-200 pb-4">
        <div className="relative">
          ...
          <input
            type="text"
            placeholder="Search Refund Policy records..."
            value={search}
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-8 bg-white rounded-3xl border border-slate-100 shadow-sm">
          ...
          <p className="text-slate-500 font-medium text-sm">Loading Refund Policy entries...</p>
        </div>
        ...
        <div className="text-center py-4 px-4 bg-white rounded-3xl border border-slate-100 shadow-sm">
          ...
          <h3 className="text-lg font-semibold text-slate-800">No Refund Policy items</h3>
          <p className="text-slate-500 text-sm max-w-md mx-auto mt-1 mb-6">
            There are no Refund Policy items created yet. Click below to add one.
          </p>
          <button
            onClick={handleOpenCreate}
          ...
          >
            <FiPlus className="w-4 h-4" /> Add Refund Policy Item
          </button>
        </div>
      ) : (
