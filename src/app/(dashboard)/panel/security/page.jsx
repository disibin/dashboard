...
export default function TeamSecurityPage() {
  ...
  const { staffData, setStaffData, staffLogout } = useContext(Context);
  ...
  useEffect(() => {
    if (staffData) {
      setIs2faActive(!!staffData.is_2fa_active);
    }
  }, [staffData]);
  ...
  const handleToggle2fa = async (newValue) => {
    setTfaLoading(true);
    try {
      const res = await axios.post('/api/staff/security', {
        action: 'toggle-2fa',
      ...
      if (res.data.success) {
        ...
        setStaffData((prev) => ({ ...prev, is_2fa_active: newValue }));
  ...
  const handleVerifyEmailCode = async (e) => {
    ...
    try {
      ...
      if (res.data.success) {
        toast.success('Staff email updated successfully!');
        setStaffData((prev) => ({ ...prev, email: res.data.newEmail, pending_email: null }));
  ...
  const handleDeleteAccount = async (e) => {
    ...
    try {
      const res = await axios.delete('/api/staff/me', { data: { password: deletePassword } });
      if (res.data.success) {
  ...
  return (
    <div className="p-4 w-full space-y-6">
      ...
      <div className="bg-white rounded-3xl p-6 border border-slate-100 shadow-sm flex items-center gap-4">
        ...
        <div>
          <h1 className="text-xl font-semibold text-slate-900">Security & Access Credentials</h1>
        ...
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            <p className="text-slate-500 text-sm mt-1 max-w-xl">
              Add extra security to your staff staff account. Require two-factor authentication when accessing sensitive management tools.
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-3xl p-7 border border-slate-100 shadow-sm space-y-5">
          ...
          <div className="p-3.5 rounded-2xl bg-secondary/10 border border-secondary/20 text-xs text-amber-900 leading-relaxed">
            ...
            <strong>current email ({staffData?.email})</strong> to confirm authorization before updating.
          </div>
          ...
          {emailStep === 'request' ? (
            ...
            <form onSubmit={handleVerifyEmailCode} className="space-y-4">
              <div className="p-3 rounded-xl bg-primary/10 border border-primary/20 text-xs text-emerald-800">
                Code sent to: <strong>{staffData?.email}</strong>. Enter code below to confirm change to{' '}
              ...
              <div className="flex gap-2">
                ...
                <button
                  type="submit"
                  disabled={emailLoading}
                  className="w-2/3 inline-flex items-center justify-center gap-2 py-3 rounded-xl bg-emerald-600 text-white font-semibold text-sm hover:bg-primary/100 transition-all shadow-md shadow-emerald-600/20 disabled:opacity-50"
                >
        ...
        <div className="md:col-span-2 bg-rose-50/50 rounded-3xl p-7 border border-rose-100 shadow-sm space-y-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-base font-semibold text-rose-900">Danger Zone — Delete Staff Account</h3>
                <p className="text-xs text-rose-600 mt-0.5">Permanently remove your staff member account from the system.</p>
              </div>
            </div>
        ...
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-md w-full p-7 border border-slate-100 shadow-2xl space-y-5 animate-in fade-in zoom-in duration-200">
            <div className="flex items-center gap-3 text-rose-600">
              ...
              <h3 className="text-lg font-semibold text-slate-900">Confirm Staff Account Deletion</h3>
            </div>

            <p className="text-sm text-slate-600 leading-relaxed">
              This action is permanent. Enter your password to remove your staff member profile. Note: If you are the last active manager, account deletion will be blocked.
            </p>
