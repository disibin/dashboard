...
import TermsOfServiceForm from '@/component/staff/forms/TermsOfServiceForm';
...
export default function TeamTermsOfServicePage() {
  ...
  const fetchTerms = async () => {
    setLoading(true);
    try {
      const res = await axios.get('/api/staff/terms-of-service');
      if (res.data.success) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingItem) {
        const res = await axios.put('/api/staff/terms-of-service', {
          id: editingItem.id,
          ...formData
        });
        if (res.data.success) {
          toast.success('Terms of Service item updated successfully');
          setShowForm(false);
          setEditingItem(null);
          fetchTerms();
        }
      } else {
        const res = await axios.post('/api/staff/terms-of-service', formData);
        if (res.data.success) {
          toast.success('Terms of Service item created successfully');
          setShowForm(false);
          fetchTerms();
        }
      }
  ...
  const handleDelete = async (id) => {
    if (!window.confirm('Are you sure you want to delete this Terms of Service item?')) return;
    setDeletingId(id);
    try {
      const res = await axios.delete(`/api/staff/terms-of-service?id=${id}`);
      if (res.data.success) {
    ...
    <div className="p-4 w-full min-h-screen pb-24">
      ...
      {loading ? (
        ...
        <div className="text-center py-16 px-4 bg-white rounded-3xl border border-slate-100 shadow-sm">
          ...
          <h3 className="text-lg font-bold text-slate-800">No Terms of Service items</h3>
          <p className="text-slate-500 text-sm max-w-md mx-auto mt-1 mb-6">
            There are no Terms of Service items created yet. Click below to add one.
          </p>
          <button
            onClick={handleOpenCreate}
          ...
          >
            <FiPlus className="w-4 h-4" /> Add Terms Item
          </button>
        </div>
      ) : (
