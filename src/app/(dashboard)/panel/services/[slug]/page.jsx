...
export default function StaffServiceDetailPage() {
  ...
  const fetchService = async () => {
    setLoading(true);
    try {
      const res = await axios.get(`/api/staff/services/${slug}`);
      if (res.data.success) {
      ...
      toast.error('Failed to load staff service details');
    } finally {
      setLoading(false);
    }
  };
  ...
  if (loading) {
    return (
      <div className="p-12 text-center text-slate-400 space-y-3 max-w-xl mx-auto">
        ...
        <p className="text-xs font-medium">Loading staff service details...</p>
      </div>
    );
  }
  ...
  if (!service) {
    return (
      <div className="p-8 max-w-xl mx-auto text-center space-y-3">
        ...
        <p className="text-xs text-slate-500">The requested service could not be found.</p>
        <Link
          href="/panel/services"
  ...
  return (
    <div className="p-4 w-full space-y-6 max-w-4xl mx-auto">
      ...
      <div className="flex items-center justify-between pb-3 border-b border-slate-200">
        ...
        >
          <FiArrowLeft size={15} /> Back to Staff Services
        </Link>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="space-y-1">
            ...
            {service.creator_name && (
              <p className="text-xs text-slate-400 pl-10">Created by {service.creator_name}</p>
            )}
          </div>
