...
export default function BoardMemberDetailPage() {
  ...
  const fetchMemberDetails = async () => {
    try {
      const res = await axios.get(`/api/staff/board/${memberId}`);
      if (res.data.success) {
  ...
  const handleSave = async (e) => {
    ...
    try {
      const res = await axios.patch(`/api/staff/board/${memberId}`, formData, {
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      const res = await axios.delete(`/api/staff/board?id=${memberId}`);
      if (res.data.success) {
