...
export default function TeamLoginLogsPage() {
  ...
  const fetchLogs = async () => {
    setLoading(true);
    try {
      const res = await axios.get('/api/staff/staff-login-logs');
      if (res.data.success) {
        setLogs(res.data.data);
      }
    } catch {
      toast.error('Failed to load staff login logs');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="bg-white p-4 rounded-3xl border border-slate-100 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="space-y-1">
          <h1 className="text-2xl font-semibold text-slate-900 flex items-center gap-2.5">
            ...
            Staff Staff Login Audit Logs
          </h1>
          <p className="text-slate-500 text-sm pl-11">
            Audit history of internal staff member authentication events and security access logs
          </p>
        </div>
