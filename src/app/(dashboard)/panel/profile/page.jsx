  ...
  const fetchProfile = async () => {
    try {
      ...
      if (res.data.success) {
        ...
        setStaffData(prev => ({ ...prev, ...res.data.data }));
      }
    } catch (error) {
      ...
      <div className="relative overflow-hidden bg-primary rounded-3xl p-8 text-white shadow-xl">
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="flex items-center gap-5">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h1 className="text-2xl font-black tracking-tight">{member?.name || 'Staff Profile'}</h1>
          ...
          <div className="flex flex-wrap items-center gap-3">
            <Link
              href="/panel/settings"
              className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-white text-slate-900 font-semibold text-sm hover:bg-slate-100 transition-all shadow-md"
            >
        ...
        <div className="bg-white rounded-3xl p-7 border border-slate-100 shadow-sm space-y-5">
          ...
          <div className="space-y-4">
            <div>
              ...
              <p className="text-sm font-semibold text-slate-800 mt-1">{member?.name || '—'}</p>
            </div>
            ...
            <div>
              <p className="text-xs font-semibold uppercase tracking-wider text-slate-400">Assigned Role</p>
              <p className="text-sm font-semibold text-slate-800 capitalize mt-1">{member?.role || '—'}</p>
            </div>

            <div>
              <p className="text-xs font-semibold uppercase tracking-wider text-slate-400">Phone Number</p>
              <p className="text-sm font-semibold text-slate-800 mt-1">{member?.phone || '— Not provided'}</p>
            </div>
          </div>
        </div>
        ...
        <div className="md:col-span-2 bg-white rounded-3xl p-7 border border-slate-100 shadow-sm space-y-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
            <div className="p-4 rounded-2xl bg-slate-50 border border-slate-100">
              ...
              <p className="text-sm font-semibold text-slate-900 mt-1">#TM-{member?.id}</p>
            </div>
            ...
            <div className="p-4 rounded-2xl bg-slate-50 border border-slate-100">
              <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Created On</p>
              <p className="text-sm font-semibold text-slate-900 mt-1">{fmtDate(member?.created_at)}</p>
            </div>

            <div className="p-4 rounded-2xl bg-slate-50 border border-slate-100">
              <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Last Login</p>
              <p className="text-sm font-semibold text-slate-900 mt-1">{fmtDate(member?.last_login)}</p>
            </div>
          </div>
        </div>
