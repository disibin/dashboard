...
import {
  FiArrowLeft, FiCheck, FiLoader, FiAlertCircle, FiBox, FiTag
} from 'react-icons/fi';
...
export default function StaffPackageDetailPage() {
  ...
  const fetchPackage = async () => {
    setLoading(true);
    try {
      const res = await axios.get(`/api/staff/packages/${slug}`);
      if (res.data.success) {
      ...
      toast.error('Failed to load staff package details');
    } finally {
      setLoading(false);
    }
  };
