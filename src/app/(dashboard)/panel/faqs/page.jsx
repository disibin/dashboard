...
import FaqForm from '@/component/staff/forms/FaqForm';
...
export default function TeamFaqsPage() {
  ...
  const fetchFaqs = async () => {
    setLoading(true);
    try {
      const res = await axios.get('/api/staff/faq');
      if (res.data.success) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingItem) {
        const res = await axios.put('/api/staff/faq', {
          id: editingItem.id,
          ...formData
        });
        if (res.data.success) {
        ...
        const res = await axios.post('/api/staff/faq', formData);
        if (res.data.success) {
  ...
  const handleDelete = async (id) => {
    ...
    setDeletingId(id);
    try {
      const res = await axios.delete(`/api/staff/faq?id=${id}`);
      if (res.data.success) {
