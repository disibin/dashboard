...
import PrivacyPolicyForm from '@/component/staff/forms/PrivacyPolicyForm';
...
export default function TeamPrivacyPolicyPage() {
  ...
  const fetchPolicies = async () => {
    setLoading(true);
    try {
      const res = await axios.get('/api/staff/privacy-policy');
      if (res.data.success) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingItem) {
        const res = await axios.put('/api/staff/privacy-policy', {
          id: editingItem.id,
          ...formData
        });
        if (res.data.success) {
        ...
        const res = await axios.post('/api/staff/privacy-policy', formData);
        if (res.data.success) {
  ...
  const handleDelete = async (id) => {
    ...
    setDeletingId(id);
    try {
      const res = await axios.delete(`/api/staff/privacy-policy?id=${id}`);
      if (res.data.success) {
  ...
  return (
    <div className="p-4 w-full mx-auto min-h-screen pb-24">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8 bg-tertiary p-4 sm:p-5 rounded-3xl text-primary shadow-sm">
        <div>
