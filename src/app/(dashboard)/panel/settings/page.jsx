...
export default function TeamSettingsPage() {
  ...
  useEffect(() => {
    if (staffData) {
      setFormData({
        ...
        address_line2: staffData.address_line2 || '',
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await axios.patch('/api/staff/me', formData);
      if (res.data.success) {
        toast.success('Staff profile updated successfully!');
        const updated = { ...staffData, ...res.data.data };
        ...
        <div className="bg-white rounded-3xl p-7 border border-slate-100 shadow-sm space-y-4">
          ...
          <div>
            ...
            <span className="text-[11px] text-slate-400 mt-1 block">
              To update email, navigate to the <a href="/panel/security" className="text-slate-800 underline">Security page</a>.
            </span>
          </div>
          ...
          <div>
            ...
            <input
              type="text"
              value={staffData?.role || 'Staff Member'}
              disabled
