...
const TeamMemberManagement = () => {
  ...
  const toggleStatus = async (member) => {
    ...
    try {
      const res = await axios.patch('/api/staff', {
        id: member.id,
  ...
  const removeMember = async (member) => {
    ...
    try {
      const res = await axios.delete(`/api/staff?id=${member.id}`);
      if (res.data.success) {
    ...
    <div className="p-6 space-y-6 min-h-screen bg-slate-50/50">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">Manage staff members, roles and active statuses</p>
        </div>
