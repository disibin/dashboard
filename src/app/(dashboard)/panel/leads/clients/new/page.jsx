...
export default function NewClientLeadPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await axios.post('/api/staff/leads/clients', form);
      if (res.data.success) {
        toast.success('Client lead created successfully!');
        router.push('/panel/leads/clients');
      } else {
  ...
  return (
    <div className="p-4 w-full space-y-4">
      ...
      <form onSubmit={handleSubmit} className="bg-white p-4 sm:p-5 rounded-2xl border border-slate-100 shadow-sm space-y-4">
        <div className="space-y-1.5">
          <label className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Client / Business Name *</label>
          <input
            type="text"
            required
            value={form.name}
        ...
        <div className="space-y-1.5">
          <label className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Address / Location</label>
          <input
            type="text"
            value={form.address}
        ...
        <div className="space-y-1.5">
          <label className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Notes & Special Requirements</label>
          <textarea
            rows={4}
            value={form.note}
        ...
        <div className="flex flex-wrap items-center justify-end gap-3 pt-4 border-t border-slate-100">
          ...
          <button
            type="submit"
            disabled={loading}
            className="px-5 py-3 rounded-xl bg-slate-900 hover:bg-primary text-white font-semibold transition-all text-sm disabled:opacity-50 flex items-center gap-2 shadow-md"
          >
