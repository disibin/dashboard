...
import {
  ...
  FiLoader, FiX, FiUserCheck, FiClock, FiRefreshCw, FiCheck
} from 'react-icons/fi';
...
export default function ClientLeadsPage() {
  ...
  const fetchLeads = async () => {
    setLoading(true);
    try {
      const res = await axios.get('/api/staff/leads/clients');
      if (res.data.success) {
  ...
  const handleSaveInlineEdit = async (id) => {
    ...
    setSavingEdit(true);
    try {
      const res = await axios.patch(`/api/staff/leads/clients/${id}`, editForm);
      if (res.data.success) {
      ...
      toast.error('Error saving client lead');
    } finally {
      setSavingEdit(false);
    }
  };
  ...
  const handleDelete = async (lead) => {
    ...
    setDeleting(lead.id);
    try {
      const res = await axios.delete(`/api/staff/leads/clients/${lead.id}`);
      if (res.data.success) {
  ...
  const autoGeneratedCount = leads.filter(l => l.note?.includes('Auto-generated') || l.note?.includes('Subscribed')).length;
    ...
    <div className="p-4 sm:p-4 md:p-5 w-full space-y-4">
      ...
      <div className="bg-white p-4 rounded-3xl border border-slate-100 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="space-y-1">
          ...
          <p className="text-slate-500 text-sm pl-11">
            Manage user registrations, website subscribers, and sales leads
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div className="bg-white p-5 rounded-3xl border border-slate-100 shadow-sm flex items-center gap-4">
          ...
          <div>
            <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Auto & Subscribers</p>
            <p className="text-2xl font-semibold text-slate-900">{loading ? '...' : autoGeneratedCount}</p>
          </div>
        </div>
        ...
        <div className="bg-white p-5 rounded-3xl border border-slate-100 shadow-sm flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-2xl font-semibold text-slate-900">{loading ? '...' : manualCount}</p>
          </div>
        </div>
      </div>
        ...
        {loading ? (
          ...
          <div className="py-4 text-center space-y-4 px-4">
            ...
            <div className="space-y-1 max-w-sm mx-auto">
              <h3 className="font-semibold text-slate-800 text-base">No client leads found</h3>
              <p className="text-xs text-slate-500">
                {search ? 'No results match your search query.' : 'Click "Add New Client Lead" to create your first lead.'}
              </p>
            </div>
            {!search && (
              <Link
                href="/panel/leads/clients/new"
                className="inline-flex items-center gap-2 px-5 py-2.5 bg-primary text-white font-semibold rounded-xl text-xs hover:bg-primary-dark transition-all shadow-md"
              >
                      ...
                      <tr key={lead.id} className="bg-primary/5">
                        ...
                        <td className="px-4 py-4 text-right space-x-1">
                          <button
                            ...
                            disabled={savingEdit}
                            className="p-2 text-white bg-primary hover:bg-primary-dark rounded-xl transition-all disabled:opacity-50"
                            title="Save Changes"
                          >
